...
class PlayroomEnvironment():
    ...
    def reset(self, state=None, true_state=True):
        ...
        # Else, randomly sample an initial state.
            ...
            self.current_state = np.array([initial_eye_item, initial_hand_item, initial_marker_item,
                                           False, False, False], dtype=float)

        return np.copy(self.current_state)

    def step(self, action, true_state=True):
        # print(INDEX_TO_ACTION[action])
        eye_item, hand_item, marker_item, light, music, bell = self.get_state_from_array(self.current_state)
        ...
        self.current_state = np.array([eye_item, hand_item, marker_item, light, music, bell], dtype=float)
    ...
    def get_state_from_array(self, state=None):
        if state is None:
            state = self.current_state
    ...
    def get_start_states(self):
        items = [0, 1, 2, 3]
        start_state_tuples = list(itertools.product(items, items, items, [False], [False], [False]))
        start_states = []
        for start_state_tuple in start_state_tuples:
            start_states.append(np.array([elm for elm in start_state_tuple]))
        return start_states
    ...
    def get_successors(self, state=None, actions=None, return_probs=False):
        ...
        for action in actions:
            eye_item, hand_item, marker_item, light, music, bell = self.get_state_from_array(state)
